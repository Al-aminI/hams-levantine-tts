...
def main():
    ...
    ap.add_argument("--c-stft", type=float, default=0.0,
                    help="multi-resolution STFT loss weight (reduces vocoder HF 'air'; try ~2)")
    ap.add_argument("--seg-size", type=int, default=8192)
    ap.add_argument("--save-every", type=int, default=2000)
    ap.add_argument("--log-every", type=int, default=50)
    ap.add_argument("--num-workers", type=int, default=4)  # Windows-safe DataLoader default
    ap.add_argument("--deterministic-duration", action="store_true",
                    help="use the deterministic duration head (fresh) — warm-start other weights from --ckpt")
    ap.add_argument("--sample-rate", type=int, default=0,
                    help="output sample rate (e.g. 24000 to fine-tune the decoder to 24 kHz)")
    ap.add_argument("--disc-ckpt", default=None,
                    help="discriminator.pt to warm-start (else looked for next to --ckpt)")
    args = ap.parse_args()

    dev = "cuda"
    if args.deterministic_duration:
        cfg = HamsVITSConfig(base_model_id=args.base, num_languages=4, num_speakers=1,
                             deterministic_duration=True, sample_rate=args.sample_rate)
        model = HamsVITS(cfg)
        if args.ckpt:
            state = torch.load(os.path.join(args.ckpt, "hams_vits.pt"), map_location="cpu")
            missing, unexpected = model.load_state_dict(state, strict=False)
            dur_missing = [k for k in missing if "duration_predictor" in k]
            print(f"[gan] warm-start from {args.ckpt}: {len(missing)} missing "
                  f"({len(dur_missing)} duration-head keys fresh), {len(unexpected)} unexpected "
                  f"| output {model.sample_rate} Hz", flush=True)
    else:
    ...
    obj = VitsObjective(model, seg_size=args.seg_size, sample_rate=model.sample_rate,
                        c_mel=args.c_mel, c_kl=args.c_kl, c_dur=args.c_dur, c_fm=args.c_fm,
                        c_stft=args.c_stft).to(dev)
    ...
    # Persist/restore the DISCRIMINATOR across warm-starts so adversarial training
    # accumulates (a fresh discriminator each run never matures -> buzzy texture).
    disc_ckpt = args.disc_ckpt or (os.path.join(args.ckpt, "discriminator.pt") if args.ckpt else None)
    if disc_ckpt and os.path.exists(disc_ckpt):
        obj.discriminator.load_state_dict(torch.load(disc_ckpt, map_location=dev))
        print(f"[gan] warm-started DISCRIMINATOR from {disc_ckpt} (adversarial training continues)", flush=True)
    else:
        print("[gan] discriminator starts FRESH (no disc checkpoint found)", flush=True)

    def _save(tag):
        sd = os.path.join(args.out, tag)
        model.save_checkpoint(sd)
        torch.save(obj.discriminator.state_dict(), os.path.join(sd, "discriminator.pt"))
        ...
        for batch in dl:
            ...
            if step % args.log_every == 0:
                ...
                print(f"[{step:>6}] g={logs['loss_g']:.2f} d={logs['loss_d']:.2f} "
                      ...
                      f"adv={logs['adv']:.2f} fm={logs['fm']:.2f} stft={logs['stft']:.2f} "
